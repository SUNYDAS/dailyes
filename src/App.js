  ...
  const onEdit = async (id, updatedTitle, updatedDesc, updatedStatus) => {
    try {
      const res = await axios.put(`${API_URL}/${id}`, {
        title: updatedTitle,
        desc: updatedDesc,
        status: updatedStatus, // 🔴 important!
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="bg-white p-6 rounded-lg shadow text-center">
          ...
          <p className="text-gray-600 mb-4"> Refresh the page and enter the correct password </p>
          <button onClick={() => window.location.reload()} className="bg-indigo-600 text-white px-4 py-2 rounded">
            Retry
          </button>
        </div>
      </div>
